    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        ...
        <el-form-item label="效果" prop="effect">
          <el-input v-model="form.effect" type="textarea" :rows="2" placeholder="请输入效果" />
        </el-form-item>
...
const categoryOptions = ['武器', '防具', '饰品', '其他']
...
async function loadData() {
  loading.value = true
  try {
    const res: any = await request.get('/equipment/list', { params: searchParams })
...
async function handleDelete(id: number) {
  try {
    await ElMessageBox.confirm('确定要删除该装备吗？', '提示', {
